...
module.exports = {
  jobPosted: () => {
    counterJobsPosted.inc(1);
  },
  jobCompleted: () => {
    counterJobsCompleted.inc(1);
  },
  jobSucceeded: () => {
    counterJobsSucceeded.inc(1);
  },
  jobFailed: () => {
    counterJobsFailed.inc(1);
  },
  jobKilled: () => {
    counterJobsKilled.inc(1);
  },

  setActiveWorkers: (amount) => {
    gaugeWorkers.set(amount);
  },
  setActiveJobPosters: (amount) => {
    gaugeJobPosters.set(amount);
  },
  setWorkerLoadCPU: (workerName, value) => {
    workerLoadCPUStats.labels(workerName).set(value);
  },
  setWorkerLoadTasks: (workerName, value) => {
    workerLoadTasksStats.labels(workerName).set(value);
  },
  setWorkerLoadOps: (workerName, value) => {
    workerLoadOpsStats.labels(workerName).set(value);
  },
  setWorkerLoadRank: (workerName, value) => {
    workerLoadRankStats.labels(workerName).set(value);
  },
